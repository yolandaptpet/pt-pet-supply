...
import { c as getCollection, d as $$Button, e as officialStaffList, o as officialStoreHours } from '../chunks/_astro_content_Dgtleold.mjs';
/* empty css                                  */
...
const $$Staff = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<section id="staff" class="relative pb-10 tracking-wider text-center overflow-hidden -mt-48 mb-8 pt-56"> <img src="src/assets/panel.png" alt="Wood paneling background" class="absolute z-[-2] bottom-0 w-full h-[85%] object-cover"> <h3 class="inline-block p-4 md:p-6 font-bold text-4xl tracking-widest bg-[#7F0201] bg-opacity-80 text-white text-outline-white rounded-3xl"${addAttribute({ backdropFilter: "blur(35px)" }, "style")}>
Meet Our Pack
